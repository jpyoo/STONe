...
def create_sliding_windows(input_data, target_data, window_size):
    ...
    for i in range(len(input_data) - window_size):
        ...
        target_value = target_data[i + window_size]
...
def create_sliding_windows_m2m(input_data, target_data, window_size, horizon = 0, pred_window = 1):
    """
    Creates sliding windows for time series data, where the target is the last element of each input sequence.
    ...
    - torch.Tensor: Corresponding target values (last element in each sequence).
    """
    assert pred_window > 0, "Pred window has to be greater than 0."
    
    input_sequences = []
    target_values = []

    #Case: num_output is one
    if target_data.ndim < 3:
        target_data = np.expand_dims(target_data, axis=-1)

    for i in range(len(input_data) - window_size - horizon - (pred_window-1)):
        input_sequence = input_data[i: i + window_size]
        
        # Target is the effective dose rate at the next time step
        target_value = target_data[i + window_size + horizon : i + window_size + horizon + pred_window]
...
def train_val_test_split(input_data, target, train_ratio = 0.8, val_ratio = 0.1):
